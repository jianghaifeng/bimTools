...
#ifndef	TTM_SETTITLE
#define TTM_SETTITLEA           (WM_USER + 32)  // wParam = TTI_*, lParam = TCHAR* szTitle
#define TTM_SETTITLEW           (WM_USER + 33)  // wParam = TTI_*, lParam = wTCHAR* szTitle
#ifdef	UNICODE
...
#ifdef	BTNST_USE_BCMENU
LRESULT CButtonST::OnMenuTCHAR(UINT nTCHAR, UINT nFlags, CMenu* pMenu) 
{
	LRESULT lResult;
	if (BCMenu::IsMenu(pMenu))
		lResult = BCMenu::FindKeyboardShortcut(nTCHAR, nFlags, pMenu);
	else
		lResult = CButton::OnMenuTCHAR(nTCHAR, nFlags, pMenu);
	return lResult;
} // End of OnMenuTCHAR
#endif
...
void CButtonST::DrawTheIcon(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	pDC->DrawState(	rImage.TopLeft(),
					...
					(bIsDisabled ? DSS_PREFIXONLY : DSS_NORMAL), 
					(CBrush*)NULL);
} // End of DrawTheIcon
